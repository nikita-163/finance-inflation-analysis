...
-- ============================================================
-- Итоговый вывод по всему EDA.
-- ============================================================

-- 1. Структура. Обе таблицы имеют ожидаемую структуру и типы данных.
--    Диапазоны дат не совпадают: bank_transactions покрывает 13 месяцев (по июнь 2026),
--    inflation — 12 месяцев (по май 2026). Это учтено как ограничение для дальнейшего 
--    сопоставления данных по месяцам (июнь 2026 не имеет пары в inflation).

-- 2. Полнота. NULL в критичных полях не выявлены, пропущенных месяцев/периодов нет 
--    в пределах имеющихся диапазонов.

-- 3. Уникальность. Технических дублей от повторного импорта не обнаружено. 
--    Найденные совпадения по ключевым полям — реальные повторные события 
--    (проверено вручную). Справочные значения (категории, продавцы, статусы, 
--    типы транзакций) согласованы, вариаций в написании не найдено.

-- 4. Распределение и аномалии. Суммы операций и кешбэк распределены неравномерно 
--    (среднее заметно выше медианы) — типично для расходов физлица. Все операции 
--    и кешбэк выше 95-го перцентиля проверены вручную и подтверждены как реальные, 
--    не ошибочные. Для inflation перцентильный анализ признан неприменимым по природе 
--    данных (временной ряд из 12 точек) — решение обосновано отдельно.

-- 5. Согласованность бизнес-логики. Суммы, даты и кешбэк логически непротиворечивы: 
--    отрицательных/нулевых сумм нет, дат в будущем относительно выгрузки нет, 
--    кешбэк нигде не превышает сумму операции и отсутствует там, где не должен быть. 
--    Показатели inflation_rate/key_rate находятся в реалистичном диапазоне.

-- Данные пригодны для дальнейшего анализа с учётом зафиксированного ограничения 
-- по несовпадению временных диапазонов между таблицами.